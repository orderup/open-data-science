...
# Fail early, fail often... Failing toooo often
#set -e
...
$PGSQL_BIN/psql -h $RSHOST -p $RSHOSTPORT -U $RSADMIN -d $RSNAME -c \
  "CREATE SCHEMA IF NOT EXISTS $RSSCHEMA;
  DROP SCHEMA IF EXISTS $RSBACKUPSCHEMA CASCADE;
  ALTER SCHEMA $RSSCHEMA RENAME TO $RSBACKUPSCHEMA;
